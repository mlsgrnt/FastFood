  ...
  emitter.on('DOMContentLoaded', () => {
    emitter.on('maps:findNearby', food => {
      ...
      fetch(
        ...
        data.json().then(json => {
          state.places = json.results.filter(
            place => (place.opening_hours ? place.opening_hours.open_now : true)
          );
    ...
    emitter.on('maps:getDetail', placeIndex => {
      ...
      fetch(
        `https://coral-biplane.glitch.me/citymapper?startcoord=${
          state.position.latitude
        },${state.position.longitude}&endcoord=${endCoord.lat},${endCoord.lng}`
      ).then(data => {
        data.json().then(json => {
